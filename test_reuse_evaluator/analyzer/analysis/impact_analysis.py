...
def double_plot():
    data = pd.read_csv(ROOT_PATH + config['test_reuse_plot'] + '/craftdroid_all_oracle_included_forplot.csv')

    fig, axes = plt.subplots(1, 2, figsize=(12, 5), sharey=True)
    analyse(data, ax=axes[0])
    data = pd.read_csv(ROOT_PATH + config['test_reuse_plot'] + '/atm_atm_oracle_included_passfree_forplot.csv')
...
def double_isolation_plot():
    data = pd.read_csv(ROOT_PATH + config['semantic_matching_results'] + '/latest_all.csv')
