...
public class ASE15Client implements GUIAnalysisClient {
  ...
  private void traverseWtgExample(WTGAnalysisOutput wtgOutput) {
    ...
    for (WTGNode node : wtg.getNodes()) {
      ...
      /*for (WTGNode node : wtg.getNodes()) {   
          Logger.verb("NNNNNNNNNNNNNNNNNNNNNode", node.toString() + " " + node.getId());
          nodeNum++;
          for (WTGEdge outEdge : node.getOutEdges()) {
              //      if(outEdge.getEventType())
              if(outEdge.getEventType().name().contains("implicit"))	continue;
              Logger.verb("Source", outEdge.getSourceNode().toString());
              
              Logger.verb("LabelType", outEdge.getEventType().name());
              
              Iterator<NNode> textNodes = outEdge.getGUIWidget().getTextNodes();
              ...
              for(String title : titles){
                  hasLabel = true;
                  Logger.verb("Label", title);
              }
              
              if(!hasLabel)	Logger.verb("Label", outEdge.getGUIWidget().toString());
              
              Logger.verb("Target", outEdge.getTargetNode().toString());
              edgeNum++;
              // print the size of push/pop sequence
              List<StackOperation> pushPopOperations = outEdge.getStackOps();
              @SuppressWarnings("unused")
