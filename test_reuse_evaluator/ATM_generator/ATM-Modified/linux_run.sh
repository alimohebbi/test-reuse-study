...
if [ $RESULT -eq 0 ]; then
  ...
  if [ -d $root/AppTestMigrator/source-scenarios ]; then
    ...
    cp $root/AppTestMigrator/activity_names ./$moduleName/src/main/assets/
    cp $root/AppTestMigrator/sm_config ./$moduleName/src/main/assets/
...
#      topackage=$(echo $activitytolaunch | awk 'BEGIN{FS=OFS="."} NF--')
      topackage=$(echo $activitytolaunch | sed 's/\.[^.]*$//' )
      testApplicationId=( $(grep -r "testApplicationId" ./$moduleName/build.gradle | awk -F' ' '{ print $2 }' | sed 's/"//g') )
      applicationId=( $(grep -r "testApplicationId" $targetapp/$moduleName/build.gradle | awk -F' ' '{ print $2 }' | sed 's/"//g') )
      if [ -z "$applicationId" ]; then
        applicationId=( $(grep -r "applicationId" $targetapp/$moduleName/build.gradle | awk -F' ' '{ print $2 }' | sed 's/"//g') )
      fi
      resourcepackage=($(grep -r "applicationId" ./$moduleName/build.gradle | awk -F' ' '{ print $2 }' | sed 's/"//g' | sed "s/'/ /g"))
      ...
      if [ -f $dbPath ]; then
        ...
        curl -i -H 'accept:application/json' -H 'content-type:application/json' -H 'Authorization:Basic bmVvNGo6bmVvNGo' -d "$POST_DATA" -XPOST "http://localhost:7474/db/data/transaction/commit" > neolog.txt
      fi
      ...
      if [ $RESULT -eq 0 ]; then
        ...
        if [ $RESULT -eq 0 ]; then
          ...
          mathOPSPath=( $(find ./$moduleName/src/androidTest -type f -name "CommonMatchingOps.java") )
          rm -r $mathOPSPath
          ...
          rm -r $utilPath
          serverUtilPath=( $(find ./$moduleName/src/androidTest -type d -name "server") )
          rm -r $serverUtilPath
          ...
          rm -f $root/AppTestMigrator/activity_names
