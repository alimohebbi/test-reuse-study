...
public class AppTestMigrator_expenseTrackerTest10_3 {
    ...
    public void apptestmigrator_expensetrackertest10_3() {
        ...
        onView(withId(R.id.et_category)).perform(click());
        
        onView(withId(R.id.et_category)).perform(click());
        
        onView(withId(R.id.tv_time)).check(matches(withText("Default")));
        
        onView(withId(R.id.et_price)).perform(replaceText("uber"), closeSoftKeyboard());
        
        onView(withId(R.id.et_title)).perform(click());
        
        onView(withId(R.id.tv_date)).check(matches(isDisplayed()));
        
        onView(withId(R.id.et_title)).perform(replaceText("500"), closeSoftKeyboard());
        
        onView(withId(R.id.tv_time)).perform(click());
        
        onView(withText("PM")).check(matches(isDisplayed()));
        
        onView(withText("OK")).check(matches(withText("52")));
